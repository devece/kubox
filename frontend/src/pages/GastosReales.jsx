...
export default function Gastos({ user }) {
  ...
  const generarGastoDesdeReales = async () => {
    ...
    try {
      ...
      alert(`âœ… Gasto generado: Total $${res.data.monto_total.toLocaleString()} (${res.data.unidades} unidades)`);
      cargarGastos();
    ...
    <div className="space-y-6">
      ...
      {config && (
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          <h3 className="font-semibold text-blue-800 mb-2">ConfiguraciÃ³n actual</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><span className="text-gray-600">Valor mÂ²:</span> <span className="font-bold">${config.valor_metro_cuadrado}</span></div>
            <div><span className="text-gray-600">Fondo reserva:</span> <span className="font-bold">{config.porcentaje_fondo_reserva}%</span></div>
            <div><span className="text-gray-600">InterÃ©s mora:</span> <span className="font-bold">{config.interes_mora_mensual}% mensual</span></div>
            <div><span className="text-gray-600">DÃ­as gracia:</span> <span className="font-bold">{config.dias_gracia} dÃ­as</span></div>
          </div>
        </div>
      )}

      {/* SECCIÃ“N 1: INGRESAR GASTOS REALES */}
      {esAdmin && (
        <div className="bg-white p-6 rounded-xl shadow">
          <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 text-blue-800">ðŸ“ Paso 1: Ingresar gastos reales del periodo</h2>
      ...
      {/* SECCIÃ“N 2: GENERAR GASTO MENSUAL */}
      {esAdmin && (
        <div className="bg-white p-6 rounded-xl shadow">
          <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 text-green-800">ðŸŽ¯ Paso 2: Generar cobro a residentes</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <input 
              type="month" 
          ...
          >
            {loading ? 'Generando...' : 'ðŸ’° Generar gasto del mes desde gastos reales'}
          </button>
        </div>
      )}

      {/* SECCIÃ“N 3: HISTORIAL DE GASTOS */}
      <div className="bg-white rounded-xl shadow overflow-hidden">
        <h2 className="text-xl font-semibold p-4 border-b">ðŸ“‹ Historial de gastos generados</h2>
        <div className="divide-y">
                      ...
                      <tbody>
                        {detalleActual.map(d => (
                          <tr key={d.id} className="border-t">
                            <td className="p-2">{d.unidad_numero} (<span className="text-xs">{d.metros_cuadrados || 50}mÂ²</span>)</td>
                            ...
                            <td className="p-2 text-center">
                              {d.pagado ? <span className="text-green-600">âœ… Pagado</span> : <span className="text-red-500">âš ï¸ Debe</span>}
          ...
          {gastos.length === 0 && <div className="p-8 text-center text-gray-500">No hay gastos generados aÃºn</div>}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-96">
            ...
            <div className="space-y-3">
              <div><label className="block text-sm font-medium">Valor por mÂ² ($)</label><input type="number" className="w-full p-2 border rounded" value={formConfig.valor_metro_cuadrado} onChange={e => setFormConfig({...formConfig, valor_metro_cuadrado: e.target.value})} /></div>
              ...
              <div><label className="block text-sm font-medium">% InterÃ©s mora mensual</label><input type="number" className="w-full p-2 border rounded" value={formConfig.interes_mora_mensual} onChange={e => setFormConfig({...formConfig, interes_mora_mensual: e.target.value})} /></div>
              <div><label className="block text-sm font-medium">DÃ­as de gracia</label><input type="number" className="w-full p-2 border rounded" value={formConfig.dias_gracia} onChange={e => setFormConfig({...formConfig, dias_gracia: e.target.value})} /></div>
            </div>
